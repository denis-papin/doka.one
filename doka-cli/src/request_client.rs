...
impl WebServer {
    ...
    fn get_data_retry<V: de::DeserializeOwned>(&self, url: &str, token: &TokenType) -> WebResponse<V> {
        ...
        self.retry(get_data).unwrap_or_else(|_| WebResponse::from_api_error(&HTTP_CLIENT_ERROR))
    }
    ...
    fn get_binary_data(&self, url: &str, token: &TokenType) -> anyhow::Result<WebResponse<MediaBytes>> {
        ...
        let mb = MediaBytes { media_type: mime_type.to_string(), data: response.bytes()? };
    ...
    fn get_binary_data_retry(&self, url: &str, token: &TokenType) -> WebResponse<MediaBytes> {
        let get_binary_data = || -> anyhow::Result<WebResponse<MediaBytes>> { self.get_binary_data(url, token) };
        self.retry(get_binary_data).unwrap_or_else(|_| WebResponse::from_api_error(&HTTP_CLIENT_ERROR))
    }
    ...
    fn post_data_retry<U: Serialize, V: de::DeserializeOwned>(
        ...
        let post_data = || -> anyhow::Result<WebResponse<V>> { self.post_data(url, request, headers) };
        self.retry(post_data).unwrap_or_else(|_| WebResponse::from_api_error(&HTTP_CLIENT_ERROR))
    }
    ...
    fn post_bytes_retry<V: de::DeserializeOwned>(
        ...
        match self.retry(post_bytes) {
            Ok(v) => v,
            Err(e) => {
                WebResponse::from_simple(StatusCode::BAD_REQUEST.as_u16(), SimpleMessage { message: e.to_string() })
            }
        }
    }
    ...
    fn patch_data<V: de::DeserializeOwned>(&self, url: &str, token: &TokenType) -> anyhow::Result<WebResponse<V>> {
        let request_builder = reqwest::blocking::Client::new().patch(Url::parse(url)?).timeout(TIMEOUT);
    ...
    fn patch_data_retry<V: de::DeserializeOwned>(&self, url: &str, token: &TokenType) -> WebResponse<V> {
        ...
        self.retry(patch_data).unwrap_or_else(|_| WebResponse::from_api_error(&HTTP_CLIENT_ERROR))
    }
    ...
    fn delete_data<V: de::DeserializeOwned>(&self, url: &str, token: &TokenType) -> anyhow::Result<WebResponse<V>> {
        let request_builder = reqwest::blocking::Client::new().delete(Url::parse(url)?).timeout(TIMEOUT);
    ...
    fn delete_data_retry<V: de::DeserializeOwned>(&self, url: &str, token: &TokenType) -> WebResponse<V> {
        ...
        self.retry(delete_data).unwrap_or_else(|_| WebResponse::from_api_error(&HTTP_CLIENT_ERROR))
    }
...
impl KeyManagerClient {
    pub fn new(server_name: &str, port: u16) -> Self {
        Self { server: WebServer::new(server_name, port, "key-manager") }
    }
...
impl SessionManagerClient {
    pub fn new(server_name: &str, port: u16) -> Self {
        Self { server: WebServer::new(server_name, port, "session-manager") }
    }
...
impl AdminServerClient {
    pub fn new(server_name: &str, port: u16) -> Self {
        Self { server: WebServer::new(server_name, port, "admin-server") }
    }
    ...
    pub fn create_customer(&self, request: &CreateCustomerRequest, token: &str) -> WebResponse<CreateCustomerReply> {
        ...
        let headers = CustomHeaders { token_type: Token(token.to_string()), x_request_id: None, cek: None };
    ...
    pub fn login(&self, request: &LoginRequest) -> WebResponse<LoginReply> {
        ...
        let headers = CustomHeaders { token_type: TokenType::None, x_request_id: None, cek: None };
...
impl DocumentServerClient {
    pub fn new(server_name: &str, port: u16) -> Self {
        Self { server: WebServer::new(server_name, port, "document-server") }
    }
...
impl TikaServerClient {
    pub fn new(server_name: &str, port: u16) -> Self {
        Self { server: WebServer::new(server_name, port, "") }
    }
...
mod test {
    ...
    fn test_get_item() -> anyhow::Result<()> {
        ...
        let reply = client.get_item(
            5,
            "4Sw3Z9etp4C8RoSSxwU7fLJrBfDStLmgFXUmhaHQf4iLwHV2CRiQsaSdFQG6W3YeYL_54TmorxT9crTZp1UIOvBysHkpVVrkr8J3OA",
        );
        let _ = dbg!(&reply);
        println!("Reply : [{:?}]", reply);
        Ok(())
